...
it('returns the exact durable erasure status to the queue after every cleanup handler',async()=>{
 await expect(processErasureAction(form({requestId:request}))).rejects.toThrow(`redirect:/rights?erasureStatus=cleanup_pending&erasureRequestId=${request}`);expect(io.deleteUser).toHaveBeenCalledWith(subject,false);expect(io.fetch).toHaveBeenCalledTimes(4);expect(io.fetch).toHaveBeenCalledWith('https://local.invalid/functions/v1/cleanup-community-media',expect.objectContaining({method:'POST',cache:'no-store'}));
});
