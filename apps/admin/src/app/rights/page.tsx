...
export default async function RightsPage({searchParams}:{searchParams:Promise<{cursor?:string;error?:string;requestId?:string;actionRequestId?:string;erasureStatus?:string;erasureRequestId?:string}>}){
 ...
 {params.erasureRequestId&&RIGHTS_UUID.test(params.erasureRequestId)&&['completed','cleanup_pending','retryable','busy'].includes(params.erasureStatus??'')?<p role="status">Account erasure status: {(params.erasureStatus??'').replaceAll('_',' ')}.</p>:null}
