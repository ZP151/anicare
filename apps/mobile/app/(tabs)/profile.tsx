...
export default function ProfileScreen() {
  ...
  return (
    <ScreenScaffold key={showSettings?'settings':'profile'} collapseTitle pinHeading compact title={showSettings?(cn?'设置':'Settings'):(cn?'我的':'Me')} nativeAppearance refreshing={collectionLoading} onRefresh={auth.owner&&!showSettings?()=>setRefreshToken(value=>value+1):undefined} refreshLabel={cn?'刷新':'Refresh'} leading={showSettings?<BackButton onPress={()=>setShowSettings(false)}/>:undefined} trailing={!showSettings?<Pressable accessibilityRole="button" accessibilityLabel={cn?'设置':'Settings'} onPress={()=>setShowSettings(true)} style={styles.close}><AppIcon name="settings" size={21} color={colors.ink}/></Pressable>:undefined}>
      ...
      <Modal visible={auth.owner === null && showSignIn} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => { setShowSignIn(false); setEmail(''); setStatus(null); }}><ScreenScaffold pinHeading title={cn?'登录':'Sign in'} nativeAppearance>
      <View style={styles.card}>
        ...
        {editingName ? <Modal visible animationType="slide" presentationStyle="pageSheet" onRequestClose={() => {if(!savingAvatar&&!savingName){setEditingName(false);setShowAvatar(false);setPendingAvatar(null);}}}><ScreenScaffold pinHeading compact avoidKeyboard title={showAvatar?(cn?'选择头像':'Choose avatar'):(cn?'编辑个人资料':'Edit profile')} nativeAppearance><View style={styles.nameForm}>
