...
export default function PrivacyRoute(){
 ...
 {contact?<Pressable accessibilityRole="link" style={styles.choice} onPress={()=>{void Linking.openURL(contact!).catch(()=>setFailed(true));}}><Text>{cn?'联系隐私受理人员':'Contact the privacy team'}</Text></Pressable>:<Text>{cn?'当前未提供站外受理渠道。请保存回执；账户移除后无法在应用内查询进度。':'An external contact channel is not available yet. Save your receipt; in-app tracking ends after account removal.'}</Text>}
 ...
 <TextInput accessibilityLabel={cn?'请求说明':'Request details'} editable={!blocked} multiline maxLength={1000} value={detail} onChangeText={setDetail} style={styles.input}/>
