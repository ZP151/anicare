...
export default function CommunityScreen(){
 ...
 return <ScreenScaffold trailing={<Pressable accessibilityRole="button" accessibilityLabel={zh?'返回':'Back'} onPress={()=>router.canGoBack()?router.back():router.replace('/' as never)} style={{minWidth:44,minHeight:44,alignItems:'center',justifyContent:'center'}}><AppIcon name="close" color={c.actionPrimary}/></Pressable>} title={zh?'邻里讨论':'Community'} subtitle={communityName??(catId?(zh?'围绕这只猫的讨论':'Conversations about this cat'):(zh?'分享发现，一起照顾社区猫。':'Share discoveries and care for community cats.'))}>
   ...
   <Modal visible={picker} animationType="slide" presentationStyle="pageSheet" onRequestClose={()=>setPicker(false)}><ScreenScaffold title={zh?'选择社区':'Choose community'} trailing={<Pressable accessibilityRole="button" onPress={()=>setPicker(false)} style={s.touch}><Text style={s.link}>{zh?'完成':'Done'}</Text></Pressable>}>
     ...
     {scopeParent&&!search.trim()?<><Pressable accessibilityRole="button" onPress={()=>setScopeParent(null)} style={s.touch}><Text style={s.link}>{zh?'返回所有规划区':'Back to planning areas'}</Text></Pressable><Pressable accessibilityRole="button" onPress={()=>{setScope(scopeParent);setPicker(false);}} style={s.areaRow}><Text style={s.author}>{zh?'整个':'All of'} {label(scopeParent)}</Text></Pressable></>:null}
     {browseSingaporeCommunities(search,scopeParent).map(area=><Pressable accessibilityRole="button" accessibilityLabel={communityLabel(area,locale)} accessibilityState={{selected:scope===area.id}} key={area.id} onPress={()=>{if(!area.parentId){setScopeParent(area.id);setSearch('');}else{setScope(area.id);setPicker(false);}}} style={s.areaRow}><View><Text style={s.author}>{communityLabel(area,locale)}</Text>{area.parentId?<Text style={s.meta}>{label(area.parentId)} · {zh?'邻里':'Neighbourhood'}</Text>:null}</View><AppIcon name={scope===area.id?'check':'chevron'} color={c.actionPrimary}/></Pressable>)}
