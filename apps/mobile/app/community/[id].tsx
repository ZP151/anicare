...
import {PublicShareButton} from '../../src/components/PublicShareButton';
import {FollowControl} from '../../src/following/FollowControl';
import {getMyStoryCatLink} from '../../src/api/story-cat-link';
import {StoryCatLinkEditor} from '../../src/cat-story/StoryCatLinkEditor';
import { PostContext } from '../../src/community/PostContext';
import { CommunityAuthorSheet } from '../../src/community/CommunityAuthorSheet';
...
import { communitySampleTitle,communitySampleText, communitySampleAuthor } from '../../src/community/test-samples';
...
export default function CommunityDetailScreen(){
 const {id,mediaId,editCat}=useLocalSearchParams<{id:string;mediaId?:string;editCat?:string}>(),router=useRouter(),auth=useAccountSession();const {locale}=useLocale();const zh=locale==='zh-CN',c=useNativeColors(),s=styles(c);
 ...
 const [editorScope,setEditorScope]=useState<string|null>(null),[repairScope,setRepairScope]=useState<string|null>(null);const consumedEdit=useRef<string|null>(null);
 const input=useRef<TextInput>(null);
 useEffect(()=>{if(editCat==='1'&&auth.owner&&(post?.canDelete||repairScope===context.current)&&consumedEdit.current!==context.current){consumedEdit.current=context.current;setEditorScope(context.current);}},[editCat,post?.canDelete,repairScope,auth.owner,id]);
 const load=async(more=false,refresh=false,showRefresh=refresh,force=false)=>{
   if(force){generation.current++;loadingRequest.current=null;}
   if(!alive.current||auth.owner===undefined||loadingRequest.current!==null)return;const token=++generation.current,current=auth.pin();loadingRequest.current=token;if(refresh){if(showRefresh)setRefreshing(true);}else setLoading(true);setFailed(false);
   ...
   catch(error){if(auth.owner){const repair=await getMyStoryCatLink(id).catch(()=>null);if(alive.current&&token===generation.current&&await current())setRepairScope(repair?context.current:null);}if(alive.current&&token===generation.current&&await current()){setFailed(true);if(!refresh||(error instanceof Error&&error.message==='community_post_hidden')){setShowAuthor(false);setReaction(null);setCatName(undefined);setCanMessage(false);setPost(null);setExtra(null);setAvatars(new Map());setPortrait(undefined);setReplies([]);setCursor(null);}}}
   finally{if(loadingRequest.current===token)loadingRequest.current=null;if(alive.current&&token===generation.current&&await current()){setRefreshing(false);setLoading(false);}}
 };
 useEffect(()=>{alive.current=true;return()=>{alive.current=false;generation.current++;};},[]);
 useEffect(()=>{setEditorScope(null);setRepairScope(null);consumedEdit.current=null;generation.current++;busy.current=false;loadingRequest.current=null;setRefreshing(false);setLoading(true);setFailed(false);setWriting(false);setShowAuthor(false);setReaction(null);setCatName(undefined);setCanMessage(false);setPost(null);setExtra(null);setAvatars(new Map());setPortrait(undefined);setReplies([]);setCursor(null);setBody('');setNotice('');pending.current=null;if(auth.owner!==undefined)void load();},[id,auth.owner]);
 const loadRef=useRef(load);loadRef.current=load;
 // Focus revalidation must not open the native pull-to-refresh control.
 useFocusEffect(useCallback(()=>{void loadRef.current(false,true,false);},[id,auth.owner]));
 const reply=async()=>{
 ...
 const authorName=(item:CommunityPost|CommunityReply)=>communitySampleAuthor('postId' in item?item.postId:item.replyId,locale)?.name??item.author.name;
 const messageAuthor=(postId:string)=>{setShowAuthor(false);router.push(`/messages/new?type=community_post&contentId=${postId}` as never);};
 const author=(item:CommunityPost|CommunityReply,type:'community_post'|'community_reply')=><View style={s.authorRow}><CommunityAuthorAvatar id={'postId' in item?item.postId:item.replyId} avatarKey={sample(item).label?'person':item.author.avatarKey} photoUri={avatars.get('postId' in item?item.postId:item.replyId)} size={36}/><View style={{flex:1}}>{type==='community_post'?<Pressable accessibilityRole="button" accessibilityLabel={zh?'查看作者资料':'View author profile'} onPress={()=>setShowAuthor(true)} style={{minHeight:44,justifyContent:'center'}}><Text style={s.author}>{authorName(item)}</Text></Pressable>:<Text style={s.author}>{authorName(item)}</Text>}{sample(item).label?<Text style={s.meta}>{sample(item).label}</Text>:null}<Text style={s.meta}>{new Date(item.createdAt).toLocaleDateString(zh?'zh-SG':'en-SG',{month:'short',day:'numeric'})}</Text></View>{auth.owner?<CommunityContentActions type={type} id={'postId'in item?item.postId:item.replyId} canDelete={item.canDelete} onEditCat={type==='community_post'&&item.canDelete?()=>setEditorScope(context.current):undefined} zh={zh} pin={auth.pin} onChanged={()=>load()} onNotice={setNotice}/>:null}</View>;
 const header=post?<DetailHeader onBack={()=>router.canGoBack()?router.back():router.replace('/' as never)} title={authorName(post)} subtitle={sample(post).label??new Date(post.createdAt).toLocaleDateString(zh?'zh-SG':'en-SG',{month:'short',day:'numeric'})} onIdentity={()=>setShowAuthor(true)} identityLabel={zh?'查看作者资料':'View author profile'} avatar={<CommunityAuthorAvatar id={post.postId} avatarKey={post.author.avatarKey} photoUri={avatars.get(post.postId)} size={30} linkToProfile={false}/>} actions={<>{post.catId?<FollowControl key={post.catId} animalId={post.catId} compact inline/>:null}<PublicShareButton kind="post" id={post.postId} zh={zh}/>{auth.owner?<CommunityContentActions type="community_post" id={post.postId} canDelete={post.canDelete} onEditCat={post.canDelete?()=>setEditorScope(context.current):undefined} zh={zh} pin={auth.pin} onChanged={()=>load()} onNotice={setNotice}/>:null}</>}/>:undefined;
 return <ScreenScaffold compact header={header} footer={auth.owner&&post?<GlassSurface style={s.composer}><TextInput ref={input} accessibilityLabel={zh?'写回复':'Write a reply'} value={body} onChangeText={setBody} multiline maxLength={2000} editable={!writing} placeholder={zh?'写一条回复…':'Write a reply…'} placeholderTextColor={c.muted} style={s.input}/><Pressable accessibilityRole="button" accessibilityLabel={zh?'发送回复':'Send reply'} disabled={writing||!body.trim()} onPress={()=>void reply()} style={s.send}><AppIcon name="send" color={c.onAction} size={19}/></Pressable></GlassSurface>:auth.owner===null?<Pressable accessibilityRole="button" onPress={()=>router.push('/profile' as never)} style={s.touch}><Text style={s.link}>{zh?'登录后参与讨论':'Sign in to join the conversation'}</Text></Pressable>:null} refreshing={refreshing} refreshLabel={zh?'刷新':'Refresh'} onRefresh={()=>void load(false,true)} leading={<BackButton onPress={()=>router.canGoBack()?router.back():router.replace('/' as never)}/>} title={zh?'帖子':'Post'}>
   ...
   {loading&&!post?<ActivityIndicator accessibilityLabel={zh?'加载帖子':'Loading post'} color={c.actionPrimary}/>:null}
   ...
   {!post&&auth.owner&&repairScope===context.current?<Pressable accessibilityRole="button" accessibilityLabel={zh?'更正猫咪关联':'Correct cat link'} onPress={()=>setEditorScope(context.current)} style={s.touch}><Text style={s.link}>{zh?'更正猫咪关联':'Correct cat link'}</Text></Pressable>:null}
   {post?<View style={s.parent}>{extra?.media.length?<PostGallery postId={post.postId} media={extra.media} initialMediaId={typeof mediaId==='string'?mediaId:undefined}/>:null}{extra?.title?<Text style={s.title}>{communitySampleTitle(post.postId,extra.title,locale)}</Text>:null}<Text style={s.body}>{sample(post).body}</Text><PostContext catId={post.catId} catName={catName} portrait={portrait} communitySlug={post.communitySlug}/><View style={{flexDirection:'row',gap:16,alignItems:'center'}}>{reaction?<CommunityLike key={`${auth.owner}|${id}`} reaction={reaction} zh={zh} owner={auth.owner} pin={auth.pin} onChange={setReaction} onSignIn={()=>router.push('/profile' as never)} onError={()=>setNotice(zh?'点赞未完成，请重试。':'Like not completed. Please retry.')}/>:<Pressable accessibilityRole="button" accessibilityLabel={zh?'重试点赞状态':'Retry likes'} onPress={()=>void load(false,true)} style={s.touch}><Text style={s.meta}>{zh?'重试点赞状态':'Retry likes'}</Text></Pressable>}<Pressable accessibilityRole="button" accessibilityLabel={zh?'参与讨论':'Join the conversation'} onPress={()=>auth.owner?input.current?.focus():router.push('/profile' as never)} style={{minHeight:44,flexDirection:'row',gap:6,alignItems:'center'}}><AppIcon name="reply" size={20} color={c.muted}/><Text style={s.meta}>{post.replyCount}</Text></Pressable></View></View>:null}
   {post&&auth.owner&&canMessage?<Pressable accessibilityRole="button" accessibilityLabel={zh?'发送私信':'Message author'} onPress={()=>messageAuthor(post.postId)} style={s.touch}><Text style={s.link}>{zh?'发送私信':'Message author'}</Text></Pressable>:null}
