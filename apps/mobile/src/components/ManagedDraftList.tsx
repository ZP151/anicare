...
export function ManagedDraftList({items,zh,onOpen,onDelete,isCurrent}:Readonly<{items:readonly ManagedDraftItem[];zh:boolean;onOpen(id:string):void;onDelete(id:string):Promise<void>;isCurrent():Promise<boolean>}>){
 ...
 const button=(text:string,fn:()=>void,icon:AppIconName,disabled=busy)=><IconAction label={text} icon={icon} disabled={disabled} onPress={fn} danger={icon==='trash'}/>;
 ...
 return <View style={{gap:10}}>
  ...
  <View style={{flexDirection:'row',alignItems:'center',justifyContent:'space-between'}}>{button(oldest?(zh?'最早优先':'Oldest first'):(zh?'最新优先':'Newest first'),()=>setOldest(value=>!value),oldest?'expand':'collapse')}{button(selecting?(zh?'完成':'Done'):(zh?'选择':'Select'),()=>{setSelecting(value=>!value);setSelected([]);},selecting?'check':'select')}</View>
  {selecting?<View style={{flexDirection:'row',justifyContent:'space-between',alignItems:'center'}}>{button(zh?'全选':'Select all',()=>setSelected(visible.filter(item=>item.canDelete).map(item=>item.id)),'selectAll')}<Text style={{color:c.muted,fontSize:13}}>{selected.length} {zh?'项已选':'selected'}</Text>{button(zh?'删除所选':'Delete selected',remove,'trash',busy||!selected.length)}</View>:null}
