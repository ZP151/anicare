...
export async function createCommunityPost(body: string, catId: string | null, communitySlug: string | null, client?: CommunityRpcClient): Promise<string> {
  ...
  const { data, error } = await rpc.rpc('create_community_post', { p_body: body.trim(), p_cat_id: catId, p_community_slug: communitySlug, p_request_id: requestId() });
  if (error || !uuid(data)) throw new Error('community_write_failed'); return data;
}
export async function createCommunityReply(postId: string, body: string, client?: CommunityRpcClient): Promise<string> {
  const rpc = client ?? getSupabaseClient() as unknown as CommunityRpcClient | null;
  if (!rpc || !uuid(postId) || !body.trim() || body.trim().length > 2000) throw new Error('invalid_community_reply');
  const { data, error } = await rpc.rpc('create_community_reply', { p_post_id: postId, p_body: body.trim(), p_request_id: requestId() });
  if (error || !uuid(data)) throw new Error('community_write_failed'); return data;
}
...
export async function deleteCommunityPost(postId: string, client?: CommunityRpcClient): Promise<void> {
  const rpc = client ?? getSupabaseClient() as unknown as CommunityRpcClient | null;
...
export async function listCommunityReplies(postId: string, client?: CommunityRpcClient): Promise<readonly CommunityReply[]> {
  ...
  const { data, error } = await rpc.rpc('list_public_community_replies', { p_post_id: postId, p_cursor: null, p_limit: 50 });
  if (error || !Array.isArray(data)) throw new Error('community_unavailable');
  return data.map((value) => { if (!record(value) || !uuid(value.replyId) || typeof value.body !== 'string' || !timestamp(value.createdAt) || !record(value.author) || typeof value.author.name !== 'string' || typeof value.author.avatarKey !== 'string' || !uuid(value.cursor)) throw new Error('invalid_community_reply'); return value as CommunityReply; });
}
