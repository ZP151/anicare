...
export async function createCommunityCommentReply(parentReplyId:string,body:string,client?:CommunityRpcClient,pendingRequestId=requestId()):Promise<string>{const rpc=client??getSupabaseClient() as unknown as CommunityRpcClient|null;if(!rpc||!uuid(parentReplyId)||!body.trim()||body.trim().length>2000)throw new Error('invalid_community_reply');const {data,error}=await rpc.rpc('create_community_comment_reply',{p_parent_reply_id:parentReplyId,p_body:body.trim(),p_request_id:pendingRequestId});if(error||!uuid(data))throw new Error('community_write_failed');return data;}
export async function listCommunityCommentReplies(parentReplyId:string,cursor:string|null=null,client?:CommunityRpcClient){const rpc=client??getSupabaseClient() as unknown as CommunityRpcClient|null;if(!rpc||!uuid(parentReplyId)||(cursor!==null&&!uuid(cursor)))throw new Error('invalid_community_reply');const {data,error}=await rpc.rpc('list_community_comment_replies',{p_parent_reply_id:parentReplyId,p_cursor:cursor,p_limit:30});if(error||!Array.isArray(data)||data.length>30)throw new Error('community_unavailable');const items=data.map(value=>{if(!record(value)||Object.keys(value).length!==6||!uuid(value.replyId)||typeof value.body!=='string'||value.body.length<1||value.body.length>2000||!timestamp(value.createdAt)||!record(value.author)||Object.keys(value.author).length!==2||typeof value.author.name!=='string'||typeof value.author.avatarKey!=='string'||!AVATARS.has(value.author.avatarKey)||typeof value.canDelete!=='boolean'||!uuid(value.cursor))throw new Error('invalid_community_reply');return value as CommunityReply;});return{items,nextCursor:items.length===30?items.at(-1)!.cursor:null};}
export async function getCommunityCommentContext(replyId:string,client?:CommunityRpcClient){const rpc=client??getSupabaseClient() as unknown as CommunityRpcClient|null;if(!rpc||!uuid(replyId))throw new Error('community_unavailable');const {data,error}=await rpc.rpc('get_community_comment_context',{p_reply_id:replyId});if(error||!Array.isArray(data)||data.length!==1||!record(data[0])||Object.keys(data[0]).length!==3||!uuid(data[0].replyId)||!uuid(data[0].postId)||(data[0].parentReplyId!==null&&!uuid(data[0].parentReplyId)))throw new Error('community_unavailable');return data[0] as Readonly<{replyId:string;postId:string;parentReplyId:string|null}>;}
