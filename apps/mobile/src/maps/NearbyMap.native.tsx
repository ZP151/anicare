...
export function NearbyMap({
  ...
  useEffect(() => {if (!mapEnabled) {mapReady.current = false;return undefined;}setProviderUnavailable(false);if (mapReady.current) return () => { mapReady.current = false; };readinessTimer.current = setTimeout(() => {readinessTimer.current = null;setProviderUnavailable(true);}, MAP_READINESS_TIMEOUT_MS);return () => {clearReadinessTimer();mapReady.current = false;};}, [clearReadinessTimer, mapEnabled]);
