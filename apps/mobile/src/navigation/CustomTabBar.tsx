...
export function CustomTabBar({state,navigation}:CustomTabBarProps){const router=useRouter(),c=useNativeColors(),{locale}=useLocale(),insets=useSafeAreaInsets(),cn=locale==='zh-CN';const routes=state.routes.filter(route=>isRouteName(route.name));return <GlassSurface style={[styles.bar,{paddingBottom:Math.max(insets.bottom,12)}]}><View style={styles.row}>{routes.slice(0,2).map(route=><Tab key={route.key} route={route.name as RouteName} selected={state.index===state.routes.findIndex(item=>item.key===route.key)} onPress={()=>navigation.navigate(route.name)}/>) }<Pressable accessibilityRole="button" accessibilityLabel={cn?'创建':'Create'} onPress={()=>router.push('/create' as never)} style={[styles.create,{backgroundColor:c.actionPrimary}]}><Text style={styles.plus}>＋</Text></Pressable>{routes.slice(2).map(route=><Tab key={route.key} route={route.name as RouteName} selected={state.index===state.routes.findIndex(item=>item.key===route.key)} onPress={()=>navigation.navigate(route.name)}/>)}</View></GlassSurface>;}
function Tab({route,selected,onPress}:{route:keyof typeof copy;selected:boolean;onPress():void}){const c=useNativeColors(),{locale}=useLocale(),cn=locale==='zh-CN',item=copy[route];return <Pressable accessibilityRole="tab" accessibilityState={{selected}} accessibilityLabel={cn?item.zh:item.en} onPress={onPress} style={styles.tab}><AppIcon name={item.icon} color={selected?c.actionPrimary:c.muted} size={20}/><Text style={{color:selected?c.actionPrimary:c.muted,fontSize:11}}>{cn?item.zh:item.en}</Text></Pressable>}
const styles=StyleSheet.create({bar:{position:'absolute',left:0,right:0,bottom:0,borderTopLeftRadius:24,borderTopRightRadius:24,paddingHorizontal:10,paddingTop:6},row:{flexDirection:'row',alignItems:'center',justifyContent:'space-between',gap:4},tab:{flex:1,minWidth:44,minHeight:44,alignItems:'center',justifyContent:'center',gap:2},create:{width:48,height:48,borderRadius:24,alignItems:'center',justifyContent:'center',marginHorizontal:2},plus:{color:'#fff',fontSize:30,lineHeight:34,fontWeight:'400'}});
