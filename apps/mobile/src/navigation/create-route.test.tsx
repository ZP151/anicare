...
const mockReplace=jest.fn(),mockBack=jest.fn();
...
it('offers post, report, and post-draft destinations from the create sheet',async()=>{
 ...
 expect(mockReplace).toHaveBeenLastCalledWith('/community/new');
 await fireEvent.press(view.getByRole('button',{name:'Report a sighting'}));
 expect(mockReplace).toHaveBeenLastCalledWith('/report');
 await fireEvent.press(view.getByRole('button',{name:'Post drafts'}));
 expect(mockReplace).toHaveBeenLastCalledWith('/community/drafts');
 await view.unmount();
});
