...
export function MessagesInbox({requestsOnly=false}:{requestsOnly?:boolean}){
 ...
 return <ScreenScaffold compact title={title} refreshing={loading&&ready} onRefresh={()=>void loadRef.current()} trailing={<Pressable accessibilityRole="button" accessibilityLabel={requestsOnly?(cn?'返回':'Back'):(cn?'搜索会话':'Search conversations')} style={s.touch} onPress={()=>{if(requestsOnly)router.back();else{if(searching)setSearch('');setSearching(value=>!value);}}}><AppIcon name={requestsOnly?'back':'search'} size={21} color={c.ink}/></Pressable>}>
