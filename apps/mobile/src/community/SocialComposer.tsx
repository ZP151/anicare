...
export function SocialComposer(){
 ...
 return <ScreenScaffold compact avoidKeyboard title={zh?'发布帖子':'New post'} header={<View style={[s.row,{justifyContent:'space-between'}]}>
  ...
  {draft&&draft.ownerId===auth.owner?<>
   {draft.images.length>0?<ScrollView testID="composer-photos" horizontal style={{height:156,flexGrow:0,flexShrink:0}} showsHorizontalScrollIndicator contentContainerStyle={{gap:10,alignItems:'flex-start'}}>{draft.images.map((image,index)=><View key={image.id} style={{width:96,height:148}}>
