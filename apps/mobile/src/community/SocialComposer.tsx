...
export function SocialComposer(){
 ...
 return <ScreenScaffold title={zh?'发布帖子':'New post'} trailing={<Pressable accessibilityRole="button" accessibilityLabel={zh?'保存并关闭':'Save and close'} disabled={busy} onPress={()=>void close()} style={s.touch}><AppIcon name="close" color={c.ink}/></Pressable>}>
  ...
  {draft&&draft.ownerId===auth.owner?<>
   ...
   <Pressable accessibilityRole="button" accessibilityLabel={zh?'发布':'Post'} disabled={busy||!draft.body.trim()||(!draft.catId&&!draft.communitySlug)} onPress={()=>void publish()} style={[s.publish,{backgroundColor:c.actionPrimary,opacity:busy||!draft.body.trim()||(!draft.catId&&!draft.communitySlug)?0.5:1}]}>{busy?<ActivityIndicator color={c.onAction}/>:<Text style={{color:c.onAction,fontWeight:'600'}}>{zh?'发布':'Post'}</Text>}</Pressable>
  </>:null}
...
const s=StyleSheet.create({touch:{minWidth:44,minHeight:44,justifyContent:'center'},row:{flexDirection:'row',alignItems:'center',gap:8},action:{flexDirection:'row',alignItems:'center',gap:8,paddingVertical:10,paddingHorizontal:4},photo:{width:120,height:150,borderRadius:12},title:{fontSize:18,fontWeight:'600',minHeight:48,borderBottomWidth:StyleSheet.hairlineWidth},body:{fontSize:15,lineHeight:23,minHeight:120,textAlignVertical:'top'},publish:{minHeight:48,borderRadius:24,alignItems:'center',justifyContent:'center'}});
