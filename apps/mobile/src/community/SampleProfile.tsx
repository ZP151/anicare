...
function ProfileContent({id}:{id:string}){
 ...
 return <ScreenScaffold collapseTitle titleAccessory={sample?<CommunityAuthorAvatar id={sample.id} avatarKey={sample.profile.avatarKey} size={72} linkToProfile={false}/>:undefined} header={person?<DetailHeader title={personText(person.name,locale)} subtitle={area?communityLabel(area,locale):undefined} onBack={()=>router.canGoBack()?router.back():router.replace('/' as never)} avatar={sample?<CommunityAuthorAvatar id={sample.id} avatarKey={sample.profile.avatarKey} size={32} linkToProfile={false}/>:undefined}/>:undefined} compact title={person?personText(person.name,locale):(zh?'邻居资料':'Neighbour profile')} leading={<BackButton onPress={()=>router.canGoBack()?router.back():router.replace('/' as never)}/>} refreshing={loading} onRefresh={person?()=>void load():undefined}>
  {!person?<Text style={{color:c.muted}}>{zh?'这份资料不可用。':'This profile is unavailable.'}</Text>:<>
   ...
   <Text style={[s.meta,{color:c.muted}]}>{zh?'虚构社区样本':'Fictional community sample'}{area?` · ${communityLabel(area,locale)}`:''}</Text>
