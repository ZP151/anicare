...
it('has one stable profile for every post and reply persona, with independent languages',()=>{
 ...
 for(const post of [...ACTIVE_COMMUNITY_TEST_POSTS,...COMMUNITY_TEST_POSTS])for(const author of [post.profile,post.replyProfile]){
...
it('reads only matching public posts, skips hidden content and orders by published time',async()=>{
 const ids=[...ACTIVE_COMMUNITY_TEST_POSTS,...COMMUNITY_TEST_POSTS].filter(post=>post.profile.name==='Demo Mei').map(post=>post.id);
