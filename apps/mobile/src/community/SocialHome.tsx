...
import {SAMPLE_CATS_V2} from './sample-catalog-v2';
...
import {communitySampleTitle,communitySampleText,communitySampleAuthor} from './test-samples';
...
export function SocialHome(){
 ...
 const load=useCallback(async(more=false,refresh=false)=>{
  ...
  try{
   ...
   if(alive.current&&token===epoch.current&&captured===currentScope.current&&await pinned()){
    ...
    setPosts(old=>mode==='explore'?appendExplorePage(more?old:[],page.items):more?[...new Map([...old,...page.items].map(p=>[p.postId,p])).values()]:page.items);setExtras(old=>new Map([...old,...details]));setReactions(old=>new Map([...old,...likes]));setAvatars(old=>new Map([...old,...people]));setPortraits(old=>new Map([...old,...pictures]));setPageCursors(old=>({...old,[mode]:page.items.length===20?page.nextCursor:null}));
   }
 ...
 const renderCard=({item}:{item:CommunityPost})=>{
  ...
  return <View style={{width:cardWidth,backgroundColor:c.surface,borderRadius:14,overflow:'hidden',marginBottom:12}}><Pressable accessibilityRole="button" accessibilityLabel={`${zh?'打开帖子':'Open post'}: ${communitySampleTitle(item.postId,detail?.title,locale)||sample.body}`} onPress={()=>router.push(`/community/${item.postId}` as never)}>
   ...
   {first&&detail&&detail.media.length>1?<View pointerEvents="none" style={{position:'absolute',top:8,right:8,borderRadius:10,backgroundColor:'rgba(0,0,0,0.55)',paddingHorizontal:7,paddingVertical:3}}><Text style={{fontSize:11,color:'#fff'}}>{detail.media.length}{zh?' 张':' photos'}</Text></View>:null}
   <Text numberOfLines={first||portrait?2:5} style={{color:c.ink,fontSize:14,lineHeight:20,fontWeight:'600',paddingHorizontal:10,paddingTop:10,paddingBottom:6}}>{communitySampleTitle(item.postId,detail?.title,locale)||sample.body}</Text>
   {sample.label?<Text style={{fontSize:10,color:c.muted,paddingHorizontal:10}}>{sample.label}</Text>:null}
  </Pressable>{item.catId?<View style={{paddingHorizontal:6}}><PostContext catId={item.catId} catName={SAMPLE_CATS_V2.find(cat=>cat[1]===item.catId)?.[2]} portrait={portrait}/></View>:null}<View style={{paddingLeft:10,paddingRight:6,flexDirection:'row',alignItems:'center',gap:5}}><CommunityAuthorAvatar id={item.postId} size={22} avatarKey={sample.label?'person':item.author.avatarKey} photoUri={avatars.get(item.postId)}/><Text numberOfLines={1} style={{fontSize:11,color:c.muted,flex:1}}>{communitySampleAuthor(item.postId,locale)?.name??item.author.name}</Text>{reactions.get(item.postId)?<CommunityLike reaction={reactions.get(item.postId)!} zh={zh} owner={auth.owner} pin={auth.pin} onChange={next=>setReactions(old=>new Map(old).set(item.postId,next))} onSignIn={()=>router.push('/profile' as never)} onError={()=>setNotice(zh?'点赞未保存，请重试。':'Like was not saved. Retry.')}/>:null}</View></View>;
 };
 const feed=(page:Mode)=><FlatList<CommunityPost> testID={`social-home-grid-${page}`} style={{width}} data={pageScopes[page]===scopeFor(page)?pagePosts[page]:[]} numColumns={columns} key={`columns-${page}-${columns}`} keyExtractor={item=>item.postId} renderItem={renderCard} columnWrapperStyle={columns>1?{gap:10,alignItems:'flex-start'}:undefined} contentContainerStyle={{padding:12,paddingTop:insets.top+headerHeight+12,paddingBottom:120+insets.bottom,flexGrow:1}} initialNumToRender={8} windowSize={5} onEndReached={()=>{if(mode===page&&cursor&&!loading)void load(true);}} onEndReachedThreshold={.4} scrollEnabled={mode===page} refreshControl={mode===page?<RefreshControl progressViewOffset={insets.top+headerHeight} refreshing={refreshing} onRefresh={()=>void load(false,true)}/>:undefined} ListHeaderComponent={failed&&mode===page?<Pressable accessibilityRole="button" onPress={()=>void load(false,true)} style={{minHeight:44}}><Text style={{color:c.actionPrimary}}>{zh?'暂未刷新，点此重试':'Could not refresh. Tap to retry'}</Text></Pressable>:null} ListEmptyComponent={loading&&mode===page?<ActivityIndicator/>:page==='nearby'&&mode==='nearby'&&!area?<View style={{paddingTop:20}}>{locating?<Text style={{color:c.muted}}>{zh?'正在查找附近邻里…':'Finding your neighbourhood…'}</Text>:manualFallback?<><Pressable accessibilityRole="button" onPress={()=>void findNearby(true)} style={{minHeight:44,justifyContent:'center'}}><Text style={{color:c.actionPrimary}}>{zh?'重试定位':'Retry location'}</Text></Pressable><Pressable accessibilityRole="button" onPress={()=>setPicker(true)} style={{minHeight:44,justifyContent:'center'}}><Text style={{color:c.actionPrimary}}>{zh?'选择邻里，查看附近的帖子':'Choose a neighbourhood to see nearby posts'}</Text></Pressable></>:null}</View>:mode===page?<Text style={{color:c.muted,paddingTop:20}}>{zh?'这里还没有帖子。':'No posts here yet.'}</Text>:null} ListFooterComponent={loading&&pagePosts[page].length?<ActivityIndicator/>:null}/>; return <SafeAreaView edges={['left','right']} style={{flex:1,backgroundColor:c.canvas}}>
...
<View testID="social-home-header" pointerEvents="box-none" onLayout={event=>setHeaderHeight(event.nativeEvent.layout.height)} style={{position:'absolute',top:insets.top,left:0,right:0,zIndex:10,paddingHorizontal:12,paddingTop:4,paddingBottom:6,gap:6}}><View style={{flexDirection:'row',alignItems:'center',gap:12}}><GlassSurface style={{borderRadius:26,paddingHorizontal:16,flexDirection:'row',alignItems:'center',gap:18}}>{(['explore','nearby'] as const).map(value=><Pressable key={value} accessibilityRole="tab" accessibilityState={{selected:mode===value}} onPress={()=>activateMode(value)} style={{minHeight:48,justifyContent:'center',borderBottomWidth:mode===value?2:0,borderBottomColor:c.actionPrimary}}><Text style={{color:mode===value?c.ink:c.muted,fontWeight:mode===value?'700':'500',fontSize:16}}>{value==='explore'?(zh?'发现':'Explore'):(zh?'附近':'Nearby')}</Text></Pressable>)}</GlassSurface><View style={{flex:1}}/><GlassSurface interactive style={{borderRadius:24}}><Pressable accessibilityRole="button" accessibilityLabel={zh?'选择邻里':'Choose neighbourhood'} onPress={()=>setPicker(true)} style={{minWidth:44,minHeight:44,alignItems:'center',justifyContent:'center'}}><AppIcon name="filters" color={c.ink}/></Pressable></GlassSurface></View>
  {mode==='nearby'&&chosen?<GlassSurface style={{alignSelf:'flex-start',borderRadius:22}}><Pressable onPress={()=>setPicker(true)} style={{minHeight:44,paddingHorizontal:16,justifyContent:'center'}}><Text style={{color:c.actionPrimary,fontSize:13}}>{communityLabel(chosen,locale)}⌄</Text></Pressable></GlassSurface>:null}
  {notice?<Text style={{padding:12,color:c.muted}}>{notice}</Text>:null}
</View>
  <Modal visible={picker} animationType="slide" presentationStyle="pageSheet" onRequestClose={()=>setPicker(false)}><ScreenScaffold pinHeading compact title={zh?'选择邻里':'Choose neighbourhood'} trailing={<Pressable accessibilityRole="button" accessibilityLabel={zh?'关闭':'Close'} onPress={()=>setPicker(false)} style={{minWidth:44,minHeight:44,justifyContent:'center'}}><AppIcon name="close" color={c.ink}/></Pressable>}><TextInput accessibilityLabel={zh?'搜索邻里':'Search neighbourhood'} value={search} onChangeText={setSearch} placeholder={zh?'西海岸、金文泰…':'West Coast, Clementi…'} placeholderTextColor={c.muted} style={{minHeight:44,fontSize:16,color:c.ink}}/>{parent&&!search.trim()?<><Pressable onPress={()=>setParent(null)} style={{minHeight:44}}><Text style={{color:c.actionPrimary}}>{zh?'所有规划区':'All planning areas'}</Text></Pressable><Pressable onPress={()=>choose(parent)} style={{minHeight:44}}><Text style={{color:c.actionPrimary}}>{zh?'选择整个规划区':'Choose this planning area'}</Text></Pressable></>:null}{browseSingaporeCommunities(search,parent).map(item=><Pressable key={item.id} accessibilityRole="button" accessibilityLabel={communityLabel(item,locale)} onPress={()=>{if(!item.parentId&&!search.trim())setParent(item.id);else choose(item.id);}} style={{minHeight:48,justifyContent:'center'}}><Text style={{color:c.ink}}>{communityLabel(item,locale)}</Text></Pressable>)}</ScreenScaffold></Modal>
 </SafeAreaView>;
}
