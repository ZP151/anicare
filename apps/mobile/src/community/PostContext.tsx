...
export function PostContext({ catId, catName, portrait, communitySlug }: { catId?: string | null; catName?: string; portrait?: string; communitySlug?: string | null }) {
  ...
  return <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 12, alignItems: 'center' }}>
    {catId ? <Pressable accessibilityRole="button" accessibilityLabel={zh ? '查看猫咪档案' : 'View cat profile'} onPress={() => router.push(`/cat/${catId}` as never)} style={{ flexDirection: 'row', alignItems: 'center', gap: 8, minHeight: 44, paddingHorizontal: 10, borderRadius: 22, backgroundColor: c.surface, maxWidth:'100%' }}>
      {portrait ? <Image source={{ uri: portrait }} style={{ width: 28, height: 28, borderRadius: 14 }} /> : <AppIcon name="cat" size={22} color={c.muted} />}
      <Text numberOfLines={1} style={{ fontSize: 13, fontWeight: '600', color: c.ink, flexShrink:1 }}>{catName ? localizedCatName(catId,catName,locale) : (zh ? '猫咪档案' : 'Cat profile')}</Text><AppIcon name="chevron" size={12} color={c.muted} />
    </Pressable> : null}
