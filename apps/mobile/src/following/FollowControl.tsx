...
export function FollowControl({animalId,compact=false,inline=false}:{animalId:string;compact?:boolean;inline?:boolean}) {
 ...
 if(compact){
  ...
  return <GlassSurface interactive style={{borderRadius:24,overflow:'hidden'}}><Pressable accessibilityRole="button" accessibilityLabel={label} accessibilityState={{selected:following===true,busy}} disabled={busy||auth.owner===undefined||(!failed&&following===null&&auth.owner!==null)} onPress={()=>{if(auth.owner===null)router.push('/profile' as never);else if(following===null)void auth.reload();else void change();}} style={{minWidth:48,minHeight:48,alignItems:'center',justifyContent:'center',paddingHorizontal:inline?6:12}}><AppIcon name={failed?'activity':'heart'} size={inline?18:24} color={following?colors.actionPrimary:colors.ink}/>{inline?<Text numberOfLines={1} style={{color:colors.actionPrimary,fontSize:10}}>{cn?(following?'已关注猫':'关注猫'):(following?'Following cat':'Follow cat')}</Text>:null}{following&&!inline?<Text style={{color:colors.actionPrimary,fontSize:10}}>{cn?'已关注':'Following'}</Text>:null}{failed?<Text accessibilityRole="alert" style={{color:colors.danger,fontSize:11}}>{cn?'重试':'Retry'}</Text>:null}</Pressable></GlassSurface>;
 }
