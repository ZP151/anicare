...
export function ReportHub({ dependencies, locale, allDrafts = false, onClose }: Readonly<{ dependencies: ReportHubDependencies; locale: Locale; allDrafts?: boolean; onClose?:()=>void }>) {
  ...
  if(allDrafts)return <ScreenScaffold pinHeading compact title={locale==='zh-CN'?'目击草稿':'Sighting drafts'} trailing={onClose?<Pressable accessibilityRole="button" accessibilityLabel={locale==='zh-CN'?'关闭':'Close'} onPress={onClose} style={{width:44,height:44,justifyContent:'center'}}><AppIcon name="close" color={colors.ink}/></Pressable>:undefined}>
