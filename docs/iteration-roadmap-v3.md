...
2026-09-10：当前执行切片：0.3.0（6）真机反馈修复。下一步先完成线上部署、原生编译和同包验收，再根据实际卡点安排后续迭代；不把自动化通过算作产品体验通过。 已实现的范围与边界见 [v5 交付切片](plans/2026-09-09-device-feedback-social-v5.md)。安装包构建结果另记。

## 2026-09-09 当前执行点

**最新增量：0.2.1（5）** 已实现官方邻里层、西海岸 / 金文泰搜索、邻里讨论及 App icon。956 项移动端回归和完整 CI 通过；原生包及实际图标资源已核验，[IPA 已交付](ios-next-device-test.md)。详见 [邻里与图标](plans/2026-09-09-neighbourhoods-and-app-icon.md)。下一动作是进行设备验收，随后优先解决实际阻断，再补可信地址搜索及报告选点与照护地点的联动。

用户已安装 0.1.2（3）并做过初步功能测试，反馈报告恢复失败，以及地图、样本、资料和讨论需要升级。该事实优先于下文 09-08 的历史状态。本轮按 [新加坡社区切片](plans/2026-09-09-singapore-community-v3.md)集成 0.2.0（4），[v3 增量设计稿](design/ios26-review-v3/index.html)已存档；后端部署、构建和真机结果统一记入[设备测试记录](ios-next-device-test.md)。

0.2.0（4）现已完成后端部署、16 只样本导入和 IPA 校验，安装包源为 `2216b1e`，952 项移动端回归及完整 CI 通过。当前动作是用户真机验收，并据反馈开启下一小批修复；不再等待额外审计报告才安装。

下一步顺序：交付这一候选 → 用户验证报告/地图/权限/头像/讨论 → 优先修复阻碍使用的问题 → 用真实贡献反馈改进社区地图和猫连续记录。子区域浏览已完成；下一批地图可加入可信地址搜索，讨论可按使用情况补充编辑或照片；不预先建设推荐算法、私信或额外审计流水线。AI A1 继续受真实有许可数据和 A0 结果约束，不阻塞本轮上机。

日常只跑改动涉及的报告、地图、讨论或资料测试；同一候选复用一轮完整 CI、数据库检查和一次原生构建。功能通过、已部署和真机验收分别记录，测试猫不计入产品活跃指标。
...
**最新进展：**M1–M4/A0 与 Apple 地图已合入 main `267acc5`，同源码 CI 和受控候选 `34217109488` 成功。IPA 0.1.0（1）已下载并通过来源、manifest 和 SHA-256 验证，见 [候选交付记录](evidence/ios-candidate-2026-09-08.md)。公开后端配置、Windows 组件和 USB 配对已就绪；下一步由用户用 AltServer 直接签名安装，无需先装 AltStore 或配置 Google iOS key。手机安装、启动和功能验收仍待观察。

**2026-09-08 当前执行更新：**用户仅用 iPhone、普通 Apple ID，可连接当前 Windows。M5 按免费账户侧载推进；M1–M4/A0 工具已提交 PR #5，iOS 构建修复在 PR #4。新 hosted run `34211232533` 已成功，真实产物验签、promotion 和错 SHA 负向消费通过，readiness 已刷新；下述过期描述保留为先前阻碍记录。原生编译与 Windows 安装准备进行中，尚无可安装候选。只在本次候选边界补配置、构建、验签和手机核心旅程；不重复无关 hosted 部署。详见 [iPhone 续报](reviews/2026-09-08-m3-m4-a0-m5-delivery.md)。
...
- 暂缓：推送/未读、聊天、募款/商业化、精确导航、其他未选择的视觉方向、复杂任务调度、自动身份确认、无数据的 ANN/训练平台。
- 新任务进入队列必须说明它解除哪个 F 缺口、推进哪个 G 目标；不能对应者放后续列表，不打断当前实现。
- 下一动作：M5 安装已验证 IPA，先验启动/登录/地图、报告提交与重开，再验照护/关注/恢复；发现故障按受影响行为修复和回归。M4 补真实受理联系渠道；A0 收到许可数据后运行现有离线工具和人工对照。A1 仅在效果支持时开始，不做占位 AI 功能。
- M5 已取得实际 macOS 原生编译及 IPA，当前只缺手机签名安装与实际场景观察。后续文档修正复用本次候选，不重复原生编译或无变化后端部署。


## 下一次 iPhone 测试版：设计 v2 与可操作社区猫样本（2026-09-08）

产品验收目标：首次打开有猫可看、有照片可辨认；能完成档案→关注→目击/照护→重开确认。

- [x] 87 张 v2 逐页稿、源文件、图标与字体许可、材质说明已归档在项目；[设计入口](../DESIGN.md)。图稿与原生真机验收分开记录。
- [x] U1 照片及测试内容已在测试后端就绪：6 只明确标为合成测试样本的猫、4 张不同照片、3 个支持区域、无图与旧活动案例。匿名公共档案/发现/照护读取及图片校验通过；[记录](ios-next-device-test.md)。不冒充真实社区活跃量，也不作为 AI 效果数据。
- [x] U1/U2 接通照片列表、猫档案、关注与贡献入口，收起筛选，统一 Profile 与表单，采用原生玻璃控件和自适应颜色。核心流程软件回归通过；设备表现待用户测试。
- [x] U3 安装包 0.1.2（3）已生成并验证来源、版本、校验和及锁文件，构建 `34236356892`，源码 `fea7efd`。
- [ ] 用户在 iPhone 更新安装，按 [10–15 分钟测试单](ios-next-device-test.md) 验玻璃、照片、个人资料、关注、目击草稿与照护。记录实际失败再做下一批修复。

当前开发停在可测试候选边界；不把构建成功当作用户满意或产品指标达成。后续只根据真机反馈迭代本批问题，复用已通过的验证，不增加逐页发布门禁。
