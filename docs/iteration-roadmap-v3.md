...
产品验收目标：首次打开有猫可看、有照片可辨认；能完成档案→关注→目击/照护→重开确认。用户初步认可逐页布局，要求增强毛玻璃；完整 UI 实施继续以这轮设计审阅结论为准。

- [x] 设计 v2 保留页面格局，修订玻璃材质并提供全部 87 张图与材质对比；见 [设计说明](design/ios26-review-v2/review-spec.md)。此勾选只表示图稿交付，不表示原生应用已实施或上机通过。
- [ ] U1 接通可公开猫照片，复用允许公开的已有真实猫；不足时在测试环境补齐明确标注的合成样本。覆盖 6 只猫、至少 4 张独立照片、3 个支持区域以及多次公开活动；见 [样本交付计划](design/ios26-review-v2/test-samples-release.md)。线上已有数量尚未核实，尚未导入。
- [ ] U1/U2 使用同一套接口让样本可打开、关注、追加目击和照护；稳定 ID、重复导入不重复、不重置数据库。真实猫的历史不编造，合成数据不冒充社区活跃量。
- [ ] U3 同一 IPA 在用户的 iPhone 验证样本闭环与三处原生玻璃，附实际可见样本名单。复用受影响测试和批末必要回归，不新增逐页审计或多套发布门禁。

这批先补齐照片与可测试内容，再扩展功能数量。样本导入、照片权限或原生材质仍未完成时，不能把静态设计图作为功能完成证据。
