...
**最新进展：**M1–M4/A0 工具已随 PR #5 合入 main `5fe472b`，完整 CI 通过。用户选定 iOS Apple 地图：PR #6 完成适配，真实 Apple-only 依赖锁生成成功；接下来仅跑新配置原生编译、受控候选、免费账户侧载和手机核心验收。Windows 三个安装组件已齐，等待本机公开后端 key 配置及 USB/AltStore 操作。无需新增 Google Cloud 账户或配置 iOS Google key。
