...
Every path follows one lock order: account row when applicable, then sighting, media upload job, media asset, identity-assistance job, identity proposal, and review. Existing review/deletion functions are refactored to acquire and revalidate in that order. Rows within a class are locked by UUID order. Tests exercise delete-vs-complete, delete-vs-select, and delete-vs-review interleavings.
