...
0.4.1 当前配置为 32 只猫，覆盖新加坡五个规划区域。30 条档案带照片，Echo／Amber 两条保留缺图及旧活动回归；Willow／Biscuit 补齐照片。新增 8 张自然场景与 2 张虚构成年邻居头像，部分素材适度复用，32 个样本 ID 不代表 32 个独立照片身份。图片来源与摘要见 `asset-provenance.json`，也可查看 [新素材预览](gallery-v6.html)。本段是配置说明，实际导入数量以新的 deployed 回执为准。
