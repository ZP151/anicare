...
R5 扩充（部署回执待补）：C01–C24 的编号、正文及前 13 个媒体序号保持；追加 C25–C48 日常故事，社区合计 48 帖、48 条基础回复、31 个带图片的帖子，覆盖 1–6 张。人物资料为 19 个虚构角色，有独立中英文介绍、公开邻里和帖子入口；不创建 Auth 账号或伪造点赞数、在线状态。原有 8 张自然场景素材继续复用，不把它们描述成同一只猫的连续实拍。资料页只读取当前公开帖子，不将隐藏或失败内容用本地数据补回。集中用例见 [下次真机测试](../../ios-next-device-test.md)。
