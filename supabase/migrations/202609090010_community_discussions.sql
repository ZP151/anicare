...
create table private.community_report_requests (
...
create function public.create_community_moderation_report(p_content_type text,p_content_id uuid,p_reason_code text,p_detail text,p_request_id uuid) returns uuid language plpgsql volatile security definer set search_path=pg_catalog as $$
...
begin
 ...
 select author_id into v_author from private.community_target_available(p_content_type,p_content_id,v_actor); if not found then raise exception 'target_not_available' using errcode='P0001'; end if;
...
begin
 ...
 if p_action='remove_community_content' and v_report.content_type='community_post' then update public.community_posts set deleted_at=coalesce(deleted_at,pg_catalog.now()) where id=v_report.content_id; elsif p_action='remove_community_content' then update public.community_replies set deleted_at=coalesce(deleted_at,pg_catalog.now()) where id=v_report.content_id; elsif v_report.content_type='community_post' then update public.community_posts set moderation_hidden_at=null where id=v_report.content_id and deleted_at is null; else update public.community_replies set moderation_hidden_at=null where id=v_report.content_id and deleted_at is null; end if;
 ...
 return query select p_report_id,p_action,'resolved'::text,case when p_action='remove_community_content' then 'hidden' else 'public' end;
end $$;
