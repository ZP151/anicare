...
reset role;
select is((select count(*) from audit.access_audit where action = 'set_sighting_restore_hold' and request_id = '00000000-0000-4000-8000-000000000543'), 1::bigint, 'service hold change is audited once');
set local role service_role;
...
select lives_ok($$insert into private.sighting_restore_holds (sighting_id, hold_type, source_type, expires_at) values ('00000000-0000-4000-8000-000000000521', 'safety', 'safety', now() - interval '1 minute')$$, 'expired safety hold fixture is valid');
select is((select count(*) from private.sighting_restore_holds where sighting_id = '00000000-0000-4000-8000-000000000521' and released_at is null and expires_at > now()), 0::bigint, 'expired holds are not active');
set local role authenticated;
select set_config('request.jwt.claim.role', 'authenticated', true);
select set_config('request.jwt.claim.sub', '00000000-0000-4000-8000-000000000510', true);
