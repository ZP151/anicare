...
insert into public.media_assets (id, uploader_id, storage_bucket, storage_path, sha256, redaction_confirmed_at) values
  ('00000000-0000-4000-8000-000000000812', '00000000-0000-4000-8000-000000000811', 'public-media', 'legacy/owner/public-812.jpg', repeat('a', 64), now()),
  ('00000000-0000-4000-8000-000000000813', '00000000-0000-4000-8000-000000000811', 'private-evidence', 'legacy/owner/private-813.jpg', repeat('b', 64), now());
select lives_ok(
...
select is(
  (select count(*) from private.legacy_media_deletion_jobs where media_id in (
    '00000000-0000-4000-8000-000000000812', '00000000-0000-4000-000000000813'
  )),
  2::bigint,
...
select is(
  (select array_agg(storage_bucket order by storage_bucket) from private.legacy_media_deletion_jobs
    where media_id in ('00000000-0000-4000-8000-000000000812', '00000000-0000-4000-000000000813')),
...
select is(
  (select status::text from private.legacy_media_deletion_jobs where media_id = '00000000-0000-4000-000000000812'),
...
select lives_ok($test$
  ...
  ) from private.legacy_media_deletion_jobs where media_id = '00000000-0000-4000-000000000813';
$test$, 'a transient private-evidence failure is recorded for retry');
select ok(
  exists (select 1 from private.legacy_media_deletion_jobs where media_id = '00000000-0000-4000-000000000813'
...
where media_id = '00000000-0000-4000-000000000813';
...
select is(
  (select status::text from private.legacy_media_deletion_jobs where media_id = '00000000-0000-4000-000000000813'),
...
select throws_ok(
  $$insert into private.legacy_media_deletion_jobs (media_id, storage_bucket, storage_path)
      values ('00000000-0000-4000-8000-000000000814', 'public-media', '../unsafe.jpg')$$,
  '23514', null, 'outbox rejects traversal paths before a scheduler can delete them'
);
...
insert into public.moderation_reports (id, content_type, reason, risk, status, due_at) values
  ...
  ('00000000-0000-4000-000000000824', 'sighting', 'spam', 'normal', 'resolved', now()),
  ('00000000-0000-4000-000000000832', 'sighting', 'spam', 'normal', 'resolved', now());
insert into public.moderation_actions (actor_id, report_id, action, rationale, request_id, resulting_visibility) values
  ...
  ('00000000-0000-4000-000000000822', '00000000-0000-4000-000000000824', 'no_action', 'Bulk erasure action two remains auditable.', '00000000-0000-4000-000000000826', 'limited'),
  ('00000000-0000-4000-000000000831', '00000000-0000-4000-000000000832', 'no_action', 'Rollback erasure action remains auditable.', '00000000-0000-4000-000000000833', 'limited');
select lives_ok(
  $$delete from public.user_profiles where id in ('00000000-0000-4000-8000-000000000821', '00000000-0000-4000-000000000822')$$,
...
select throws_ok(
  $$update public.moderation_actions set rationale = 'Unrelated action mutation must still fail.'
      where request_id = '00000000-0000-4000-000000000825'$$,
  '42501', 'moderation_actions_append_only', 'an unrelated action mutation fails after erasure in the same transaction'
);
select throws_ok($test$
  do $body$
  begin
    delete from public.user_profiles where id = '00000000-0000-4000-000000000831';
...
select ok(
  exists (select 1 from public.user_profiles where id = '00000000-0000-4000-000000000831')
    and exists (select 1 from public.moderation_actions where request_id = '00000000-0000-4000-000000000833' and actor_id = '00000000-0000-4000-000000000831'),
...
select is(
  public.create_moderation_report('user', '00000000-0000-4000-8000-000000000842', 'spam', null, '00000000-0000-4000-000000000843'),
  '00000000-0000-4000-000000000843'::uuid,
  'an existing user report returns the opaque request outcome'
);
select is(
  public.create_moderation_report('user', '00000000-0000-4000-000000009999', 'spam', null, '00000000-0000-4000-000000000844'),
  '00000000-0000-4000-000000000844'::uuid,
...
select is_empty(
  $$select 1 from public.moderation_reports where reporter_id = '00000000-0000-4000-8000-000000000841'
      and content_type = 'user' and content_id = '00000000-0000-4000-000000009999'$$,
...
select is(
  (select count(*) from audit.access_audit where actor_id = '00000000-0000-4000-8000-000000000841'
    and action = 'create_moderation_report' and request_id = '00000000-0000-4000-000000000844'),
