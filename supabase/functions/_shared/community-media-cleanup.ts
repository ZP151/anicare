...
type Job = Readonly<{ job_id: string; media_id: string; thumb_path: string; display_path: string; claim_id: string }>;
...
function valid(job: unknown): job is Job {
  ...
  return typeof value.job_id === 'string' && uuid.test(value.job_id) && typeof value.media_id === 'string' && uuid.test(value.media_id) && typeof value.claim_id === 'string' && uuid.test(value.claim_id) && value.thumb_path === `media/${value.media_id}/thumb.jpg` && value.display_path === `media/${value.media_id}/display.jpg`;
}
