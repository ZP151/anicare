...
Deno.serve(async (request) => {
  ...
  for (const candidate of data) {
    ...
    const shouldRemoveObject = job.cleanup_action === 'remove_and_retry' || job.cleanup_action === 'remove_and_purge';
    ...
    const { error: completeError } = await service.rpc('complete_media_staging_cleanup', {
      ...
      p_cleanup_action: job.cleanup_action,
      p_object_removed: shouldRemoveObject,
    });
    if (!completeError && shouldRemoveObject) removed += 1;
  }
